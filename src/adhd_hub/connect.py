...
def render_install_sh(
    ...
    *,
    ...
    baked_sp = "1" if with_superpowers else "0"
    baked_c7 = "1" if with_context7 else "0"
    baked_ab = "1" if with_agent_browser else "0"
    baked_se = "1" if with_serena else "0"
    return f"""#!/bin/sh
...
#   ADHD_HUB_CONNECT_WITH_RTK=1, ADHD_HUB_CONNECT_WITH_SUPERPOWERS=1,
...
WITH_RTK={baked_rtk}
WITH_SUPERPOWERS={baked_sp}
WITH_CONTEXT7={baked_c7}
WITH_AGENT_BROWSER={baked_ab}
...
case "${{ADHD_HUB_CONNECT_WITH_SUPERPOWERS:-}}" in 1|true|TRUE|yes|YES) WITH_SUPERPOWERS=1 ;; esac
case "${{ADHD_HUB_CONNECT_WITH_CONTEXT7:-}}" in 1|true|TRUE|yes|YES) WITH_CONTEXT7=1 ;; esac
case "${{ADHD_HUB_CONNECT_WITH_AGENT_BROWSER:-}}" in 1|true|TRUE|yes|YES) WITH_AGENT_BROWSER=1 ;; esac
case "${{ADHD_HUB_CONNECT_WITH_SERENA:-}}" in 1|true|TRUE|yes|YES) WITH_SERENA=1 ;; esac
...
[ "$WITH_SUPERPOWERS" -eq 1 ] && set -- "$@" --with-superpowers
[ "$WITH_CONTEXT7" -eq 1 ] && set -- "$@" --with-context7
[ "$WITH_AGENT_BROWSER" -eq 1 ] && set -- "$@" --with-agent-browser
[ "$WITH_SERENA" -eq 1 ] && set -- "$@" --with-serena
...
def render_install_ps1(
    ...
    *,
    ...
    sw_sp = "$true" if with_superpowers else "$false"
    sw_c7 = "$true" if with_context7 else "$false"
    sw_ab = "$true" if with_agent_browser else "$false"
    sw_se = "$true" if with_serena else "$false"
...
param(
  ...
  [switch]$WithGraphify,
  [switch]$WithRtk,
  [switch]$WithSuperpowers,
  [switch]$WithContext7,
  [switch]$WithAgentBrowser,
  [switch]$WithSerena,
  [switch]$DryRun
)
...
if (-not $WithSuperpowers -and {sw_sp}) {{ $WithSuperpowers = $true }}
if (-not $WithContext7 -and {sw_c7}) {{ $WithContext7 = $true }}
if (-not $WithAgentBrowser -and {sw_ab}) {{ $WithAgentBrowser = $true }}
if (-not $WithSerena -and {sw_se}) {{ $WithSerena = $true }}
...
if (-not $WithSuperpowers -and (Test-EnvFlag "ADHD_HUB_CONNECT_WITH_SUPERPOWERS")) {{ $WithSuperpowers = $true }}
if (-not $WithContext7 -and (Test-EnvFlag "ADHD_HUB_CONNECT_WITH_CONTEXT7")) {{ $WithContext7 = $true }}
if (-not $WithAgentBrowser -and (Test-EnvFlag "ADHD_HUB_CONNECT_WITH_AGENT_BROWSER")) {{ $WithAgentBrowser = $true }}
if (-not $WithSerena -and (Test-EnvFlag "ADHD_HUB_CONNECT_WITH_SERENA")) {{ $WithSerena = $true }}
...
if ($WithSuperpowers) {{ $connectArgs += "--with-superpowers" }}
if ($WithContext7) {{ $connectArgs += "--with-context7" }}
if ($WithAgentBrowser) {{ $connectArgs += "--with-agent-browser" }}
if ($WithSerena) {{ $connectArgs += "--with-serena" }}
