...
if (Get-Command adhd-hub -ErrorAction SilentlyContinue) {{
  & adhd-hub connect $Project --hub $HubUrl --agents cursor --skills --cursor-rule
  exit $LASTEXITCODE
}}

if (Get-Command uvx -ErrorAction SilentlyContinue) {{
  & uvx --from adhd-hub adhd-hub connect $Project --hub $HubUrl --agents cursor --skills --cursor-rule
  exit $LASTEXITCODE
}}
...
Install the ADHD Hub CLI first (uv tool install adhd-hub), then re-run:
  ...
  iex "& {{ $(irm $HubUrl/install.ps1) }} -Project '$Project'"
