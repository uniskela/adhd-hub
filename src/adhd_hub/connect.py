...
def render_install_sh(hub_url: str, *, default_agents: str = "") -> str:
...
# Default agents come from Hub Settings → Connections (prefs), then env.
set -eu
...
def render_install_ps1(hub_url: str, *, default_agents: str = "") -> str:
...
def print_report(report: ConnectReport) -> None:
    ...
    print()
    if report.ok:
        print("Complete! ADHD Hub is connected.")
    else:
        print("Connect finished with errors — see summary below.")
    ...
    if report.ok:
        ...
        if any(s.name == "setup complete" for s in report.steps):
            print("  - Start (or reopen) Cursor/Codex so MCP + skills reload.")
