...
A follow-up adds a separate dashboard password, token-based recovery, and login throttling. See [authentication](authentication.md) and [dashboard preferences](dashboard.md). Individual accounts and OAuth providers are not configured. Browser sessions are held in memory, so use a single worker and expect to sign in again after a restart. Use HTTPS for remote access and configure trusted proxy headers when TLS terminates upstream. Default-token unauthenticated development remains available on a loopback bind. No production deployment or external hub state was changed by this review.
