...
Open Hub pages listen for authenticated live updates (SSE) and refresh relevant data shortly after Hub-side changes; if the live connection drops, ordinary Refresh / API use still works. The installable PWA caches the UI shell only. When a newer shell service worker is waiting, a calm sticky toast offers **Refresh** (activates the update and reloads this tab); it does not interrupt mid-task on its own. Under **Settings → Forge**, **Sync health** shows last successful or failed forge reconciliation, concise errors, Needs review conflicts, recent change history from the activity ledger, and a Retry sync path beside Forge jobs.
