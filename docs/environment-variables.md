...
On My work, **More options → Rewrite scan line** forces a refresh for one thread (`POST /api/threads/{id}/scan-line`). With a project selected, **Rewrite all scan lines** in the project header confirms, then rewrites every open thread in that project (`POST /api/projects/{slug}/scan-lines`) with a short delay between calls; when AI is off the request explains and no-ops. Automatic rewrites still run on thread/progress save when AI is active.
