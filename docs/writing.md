...
Hub threads store these as structured fields (`goal`, `focus`, `next_steps`, `blocked_reason`, `resume_step`) plus a short title. Prefer updating those fields via `upsert_progress(thread_id=...)` instead of pasting a full diary every checkpoint. On routine checkpoints, omit `content` entirely — never write ceremony lines such as “Thread upserted from …”; reserve freeform `content` for rare human-meaningful events (decision, blocker, ship note). The dashboard shows those fields in [Notes & context](notes.md).
