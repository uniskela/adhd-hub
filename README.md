...
**Cloud / remote mailbox:** enable **Import cloud-agent issues (inbox)** so the Hub polls open `adhd-hub` issues from Cursor Cloud, Codex/ChatGPT, Claude, etc., creates threads, then closes them with `adhd-hub-synced` (never deletes). Optional `source:*` labels record the tool. See [docs/forge-issue-inbox.md](docs/forge-issue-inbox.md).
