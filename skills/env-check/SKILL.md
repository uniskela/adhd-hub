...
description: >-
  ...
  constraints. Use at session start when choosing tests, credentials, or Hub
  continuity fallback. Hub MCP unavailable (not runtime alone) triggers the
  forge `[ADHD]` issue mailbox; never invent Hub state.
---
...
- Fallback: when issue-write access exists and the identity is on Hub **Inbox authors**, open/update a forge/GitHub issue titled `[ADHD] …` with Goal / Focus / Next / Resume (or Now / Done / Next / Return). Optional labels `adhd-hub`, `project:<slug>`, `source:cursor` — skip labels if the token cannot set them. No secrets, private Hub URLs, internal hosts/IPs, absolute local paths, or transcripts
...
1. **MCP unavailable?** → loud first-line warning + forge `[ADHD]` fallback (if allowed) — this is the Hub continuity trigger
2. **Runtime env** → choose tests, credentials, and tooling constraints
